...
public class Relation implements Serializable {
	...
	protected Relation(SchemaManager schema_manager, int schema_index,
			String relation_name, MainMemory mem, Disk disk) {
	...
	public boolean getBlock(int relation_block_index, int memory_block_index) {
		if (memory_block_index < 0
			...
			System.err.print("getBlock ERROR: block index " + memory_block_index
					+ " out of bound in memory" + "\n");
			return false;
		}
	...
	// stores in the memory
	// returns false if the index is out of bound
	public boolean getBlocks(int relation_block_index, int memory_block_index,
			int num_blocks) {
		if (num_blocks <= 0) {
			System.err.print("getBlocks ERROR: num of blocks " + num_blocks
					+ " too few" + "\n");
			return false;
		}
		if (memory_block_index < 0
		...
		int i;
		if ((i = memory_block_index + num_blocks
				- 1) >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
			System.err.print("getBlocks ERROR: access to block "
					+ "out of memory bound: " + i + "\n");
			return false;
		}
		ArrayList<Block> v = disk.getBlocks(schema_index, relation_block_index,
				num_blocks);
	...
	public boolean setBlock(int relation_block_index, int memory_block_index) {
		if (memory_block_index < 0
				|| memory_block_index >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
			System.err.print("setBlock ERROR: block index" + memory_block_index
					+ " out of bound in memory" + "\n");
			return false;
		}
		if (relation_block_index < 0) {
			System.err
					.print("setBlock ERROR: block index " + relation_block_index
							+ " out of bound in relation" + "\n");
			return false;
		}
		...
		for (int i = 0; i < v.size(); i++) {
			if (!v.get(i).getSchema().equals(s)) {
				...
				System.out.println(v.get(i).getSchema());
				System.out.println(s);
				System.err.print("setBlock ERROR: The tuple at offest " + i
						+ " of memory block " + memory_block_index
	...
	// reads several blocks from the memory and stores on the disk
	// returns false if the index is out of bound
	public boolean setBlocks(int relation_block_index, int memory_block_index,
			int num_blocks) {
		if (num_blocks <= 0) {
			System.err.print("setBlocks ERROR: num of blocks " + num_blocks
					+ " too few" + "\n");
			return false;
		}
		if (memory_block_index < 0
		...
		if (relation_block_index < 0) {
			System.err.print(
					"setBlocks ERROR: block index " + relation_block_index
							+ " out of bound in relation" + "\n");
			return false;
		}
		...
		for (j = memory_block_index; j < memory_block_index + num_blocks; j++) {
			...
			for (k = 0; k < v.size(); k++) {
				if (!v.get(k).getSchema().equals(s)) {
					System.err.print("setBlocks ERROR: The tuple at offest " + k
							+ " of memory block " + j
							+ " has a different schema." + "\n");
					return false;
				}
		...
		Tuple t = new Tuple(schema_manager, schema_index);
		t.invalidate(); // invalidates the tuple
		if (disk.extendTrack(schema_index, relation_block_index + num_blocks,
				t)) {
			// Actual writing on disk

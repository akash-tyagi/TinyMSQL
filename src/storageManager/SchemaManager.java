...
public class SchemaManager implements Serializable {
	...
	protected Schema[] schemas = new Schema[MAX_NUM_CREATING_RELATIONS];
	private int offset;
	...
	public Schema getSchema(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("getSchema ERROR: relation " + relation_name
					+ " does not exist" + "\n");
			return new Schema();
		} else {
			return new Schema(
					schemas[relation_name_to_index.get(relation_name)]);
		}
	}
	...
	public Relation createRelation(String relation_name, Schema schema) {
		if (relation_name == "") {
			System.err
					.print("createRelation ERROR: empty relation name" + "\n");
			return null;
		}
		if (relation_name_to_index.containsKey(relation_name)) {
		...
		relations[offset] = new Relation(this, offset, relation_name, mem,
				disk);
	...
	public Relation getRelation(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("getRelation ERROR: relation " + relation_name
					+ " does not exist" + "\n");
			return null;
		} else {
	...
	public boolean deleteRelation(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("deleteRelation ERROR: relation " + relation_name
					+ " does not exist" + "\n");
			return false;
		}

...
public class OnePassUtils {
	...
	public static Relation onePassJoin(DbManager dbManager,
		...
		GeneralUtils.sortMainMemory(dbManager, commonCols,
		...
		for (int i = 0; i < secondRel.getNumOfBlocks(); i++) {
			...
			for (int j = 0; j < memIndex; j++) {
				joinBlocksData(dbManager.mem, r1, r2, dbManager.mem.getBlock(j),
						dbManager.mem.getBlock(memIndex),
						one_pass_temp_relation, storeOutputToDisk, commonCols,
						memIndex);
			}
		}
				...
				for (Tuple t1 : blockTuples) {
					if (!t.toString().equals(t1.toString())) {
						...
						GeneralUtils.appendTupleToRelation(
								sorted_unique_relation, dbManager.mem,
								dbManager.mem.getMemorySize() - 1, t);
					}
				}
			}

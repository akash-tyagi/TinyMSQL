...
public class DeleteStmt extends StmtBase implements StmtInterface {
            ...
            for (int j = 0; j < block_reference.getNumTuples(); j++) {
                ...
                if (!(cond == null || cond.execute(tuple))) {
                    ...
                    for (String field_name : field_names) {
                        String field_value = tuple.getField(field_name).toString();
                        
                        // If the field value was stored null
                        // Default value (when "NULL" is passed) for integer is Integer.MIN_VALUE 
                        // and for str is null
                        FieldType field_type = schema.getFieldType(field_name);
                        if (field_type == FieldType.INT) {
                            if (Integer.parseInt(field_value) == Integer.MIN_VALUE) {
                                field_value = "NULL";
                            }
                        } else {
    ...
    private void updateVTable(String field_name, String field_value) {
        int curValue = dbManager.vTable.get(tableName).get(field_name).get(field_value);
        curValue--;
        if (curValue == 0) {
            dbManager.vTable.get(tableName).get(field_name).remove(field_value);
        } else {
            dbManager.vTable.get(tableName).get(field_name).put(field_value, curValue);
        }
    }

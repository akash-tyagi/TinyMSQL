...
public class DeleteStmt extends StmtBase implements StmtInterface {
    ...
    @Override
    public void execute() {
        Block last_block = dbManager.mem.getBlock(dbManager.mem.getMemorySize() - 1);
        last_block.clear();
        int diskPointer = 0;
        ...
        for (int i = 0; i < relation.getNumOfBlocks(); i++) {
            ...
            for (int j = 0; j < block_reference.getNumTuples(); j++) {
                ...
                if (!(cond == null || cond.execute(tuple))) {
                    boolean blockWrittenToDisk = saveTupleToLastBlock(tuple, last_block, relation, diskPointer, dbManager.mem.getMemorySize() - 1);
    ...
    private boolean saveTupleToLastBlock(Tuple tuple, Block last_block, Relation relation, int diskPointer, int temp_location) {

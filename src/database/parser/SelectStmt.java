...
public class SelectStmt extends StmtBase implements StmtInterface {
	...
	private void parseFromClause(String rawFromClause) {
		...
		if (matcher.find()) {
			...
			if (GlobalVariable.isTestExecution)
				System.out.println("SELECT Statement: rawTableList:"
						+ rawTableList + " rawWhereCond:" + rawSearchCond
						+ " Orderby:" + orderBy);
	...
	public void execute() {
		...
		// Sorting test
		// TwoPassUtils.sortRelationForPhase1(dbManager, "course", new
		// ArrayList<>(Arrays.asList("homework", "grade")));
		productOperation();
	}
